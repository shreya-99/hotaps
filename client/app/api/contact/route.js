...
export async function POST(request) {
  try {
    ...
    if (error) return NextResponse.json({ success: false, message: error }, { status: 422 })
    ...
    return NextResponse.json({ success: false, message: 'Failed to send message. Please try WhatsApp instead.' }, { status: 500 })
  }
}
